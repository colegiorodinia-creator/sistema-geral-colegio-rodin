...
from generate_supabase_seed import sql_str, sql_date, sql_num, sql_int
...
classes = json.loads(text[pos_c + len('export const ALL_CLASSES_2027 = '):pos_s].strip().rstrip(';'))
students = json.loads(text[pos_s + len('export const ALL_STUDENTS_2027 = '):pos_e].strip().rstrip(';'))
enrollments = json.loads(text[pos_e + len('export const ALL_ENROLLMENTS_2027 = '):].strip().rstrip(';'))

# 1. SCHEMA
part1 = """CREATE EXTENSION IF NOT EXISTS "uuid-ossp";
...
with open('supabase/part1_schema.sql', 'w', encoding='utf-8') as f:
    f.write(part1)

# 2. PROFILES, CLASSES, FIXED RATES
part2 = []
part2.append("""INSERT INTO profiles (id, name, email, role, role_label, avatar_url) VALUES
...
part2.append(f"INSERT INTO classes (id, code_id, name, grade_level, academic_year, room_code, student_count) VALUES\n" + ",\n".join(class_rows) + "\nON CONFLICT (code_id) DO NOTHING;\n")

with open('supabase/part2_profiles_classes_rates.sql', 'w', encoding='utf-8') as f:
    f.write("\n".join(part2))
...
for s in students:
    ...
    for idx, g in enumerate(s.get('guardians', [])):
        ...
        if g_key not in guardian_dict:
            ...
            guardian_dict[g_key] = {
                "id": g_uuid,
                "name": name or "Responsável Legal",
...
for i in range(0, len(guardian_list), chunk_size):
    ...
    part3.append("INSERT INTO guardians (id, name, kinship_relation, cpf, rg, rg_issuer, birth_date, occupation, marital_status, nationality, email, phone_mobile, phone_landline, address_cep, address_street, address_number, address_complement, address_neighborhood, address_city, address_state) VALUES\n" + ",\n".join(g_rows) + "\nON CONFLICT (id) DO NOTHING;\n")

with open('supabase/part3_guardians.sql', 'w', encoding='utf-8') as f:
    f.write("\n".join(part3))
...
for i in range(0, len(students), chunk_size):
    ...
    part4.append("INSERT INTO students (id, rm_number, coc_code, name, enrollment_code, gender, birth_date, birth_city, nationality, rg, rg_issuer, rg_issue_date, cpf, student_phone, course_level, current_grade, class_group, school_shift, school_unit, special_needs_desc, medical_allergies, emergency_contact, photo_url, attendance_rate) VALUES\n" + ",\n".join(s_rows) + "\nON CONFLICT (rm_number) DO NOTHING;\n")

with open('supabase/part4_students.sql', 'w', encoding='utf-8') as f:
    f.write("\n".join(part4))

# 5. STUDENT GUARDIANS
unique_links = list({f"{l[0]}_{l[1]}": l for l in student_guardian_links}.values())
part5 = []
for i in range(0, len(unique_links), 250):
    chunk = unique_links[i:i + 250]
    l_rows = [f"('{l[0]}', '{l[1]}', {l[2]}, {l[3]})" for l in chunk]
    part5.append("INSERT INTO student_guardians (student_id, guardian_id, is_financial_responsible, is_pedagogical_responsible) VALUES\n" + ",\n".join(l_rows) + "\nON CONFLICT (student_id, guardian_id) DO NOTHING;\n")

with open('supabase/part5_student_guardians.sql', 'w', encoding='utf-8') as f:
    f.write("\n".join(part5))

# 6. ENROLLMENTS & CONTRACTS
part6 = []
for i in range(0, len(enrollments), chunk_size):
    ...
    part6.append("INSERT INTO enrollments (id, enrollment_code, student_id, academic_year, course_level, current_grade, new_grade, class_group, status, school_contract_status, material_contract_status) VALUES\n" + ",\n".join(e_rows) + "\nON CONFLICT (id) DO NOTHING;\n")
    part6.append("INSERT INTO contracts (id, contract_code, enrollment_id, student_id, document_sha256, terms_version, status, tuition_gross_total, tuition_nominal_total, tuition_discount_total, tuition_discount_percentage, tuition_discount_reason, tuition_discount_type, installments_count, first_installment_value, regular_installment_value, material_total_value, material_total_extenso, material_installments_count, material_installment_value, material_installment_extenso, material_start_due_date, material_end_due_date) VALUES\n" + ",\n".join(c_rows) + "\nON CONFLICT (contract_code) DO NOTHING;\n")

with open('supabase/part6_enrollments_contracts.sql', 'w', encoding='utf-8') as f:
    f.write("\n".join(part6))

print("Todas as 6 partes geradas com sucesso!")
